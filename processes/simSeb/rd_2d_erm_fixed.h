...
public:
    ...
    void init (vector<morph::Gdisplay>& displays, bool useSavedGenetics = false) {
        ...
        DBG ("called");
        // Create a HexGrid
        this->hg = new HexGrid (0.005, 2, 0, morph::HexDomainShape::Boundary);
        // Read the curves which make a boundary
	// ReadCurves r("./trial.svg");
	morph::CirclePath r(this->radius);
    ...
    /*!
     * Computes the Laplacian for c
     * Stable with dt = 0.0001;
     */
    void compute_clapl (vector<double>& fa, unsigned int i) {
...
#pragma omp parallel for schedule(dynamic,50)
	    ...
	    //if it is a boundary hex set its centre to 2.5
	    if (h->boundaryHex == true)
	      fa[h->vi] = 2.5;
            ...
            if (h->has_ne) {
                ...
                thesum += 2.5; // A ghost neighbour-east with same value as Hex_0
            }
            if (h->has_nne) {
                thesum += fa[h->nne->vi];
            } else {
    ...
    /*!
     * Computes the Laplacian for n
     * Stable with dt = 0.0001;
     */
    void compute_nclapl (vector<double>& fa, unsigned int i) {
...
#pragma omp parallel for schedule(dynamic,50)
        for (unsigned int hi=0; hi<this->nhex; ++hi) {
            ...
            Hex* h = this->hg->vhexen[hi];
	    // if it is a boundary hex set to fixed point solution
	    if (h->boundaryHex == true)
	      fa[h->vi] = 1.0;
            ...
            if (h->has_ne) {
                ...
                thesum += 1.0; // A ghost neighbour-east with same value as Hex_0
            }
            if (h->has_nne) {
                thesum += fa[h->nne->vi];
            } else {
...
#pragma omp parallel for schedule(dynamic,50)
	    ...
	    if (h->boundaryHex == true) {
	      ...
	      fa2(h->vi] = 2.5;
            ...
            vector<double> dum1(6,1.0);
            vector<double> dum2(6,2.5);
  ...
  int find_maxTh(vector<double> ray) {
    ofstream cfile ( "./logs/w0/angleTurn.txt" );
    int iend = ray.size();
    cfile<<"iend = " << iend <<endl;
    turnVal.resize(100);
    cout <<"ray size = "<<iend<<endl<<flush;
    double old_slope = 0;
    double new_slope = 0;
    int count = 0;
    old_slope = (ray[0] - ray[iend-1]) / fabs(ray[0] -ray[iend-1]);
    for (int i =1; i<=iend;i++){
      
      new_slope = (ray[i%iend]-ray[(i-1)%iend]) / fabs((ray[i%iend]-ray[(i-1)%iend]));
      if (new_slope*old_slope < 0.000001) {
	turnVal[count].radialIndex = i%iend;
...
//sectorize over radius
  vector <double> sectorize_radius (int numSectors) {
    ...
    ofstream dfile ( "./logs/w0/sectorRadius.txt" );
    //string fname = this->logpath + "sectorRadius.txt";
    // ofstream dfile ( fname );
    DBG ("In sectorize_radius");
    double maxRadius = this->radius;
    vector <double>  radiusCC;
    radiusCC.resize(numSectors+1,0);
    ...
    endAngle = 7*angleInc;
     ...
     for (int k=0;k<numSectors;k++) { 
       ...
       list<morph::Hex>::iterator bpi = this->hg->hexen.begin(); 
      
      while (bpi != this->hg->hexen.end()) { 
       if (bpi->phi >= startAngle && bpi->phi < endAngle) { 
    	 if (bpi->r >= startRadius && bpi->r < endRadius) { 
     	      radiusCount[k]++; 
	      radiusCC[k] = radiusCC[k] + this->c[0][bpi->vi]; 
	 } //end of if on radius 
       } //end of if on angle 
     	bpi++; 
      } //end of loop over bpi
     ...
     radiusCC[numSectors] = radiusCC[numSectors-2];
      return radiusCC; 
  ...
  vector <double> sectorize_angle (int numSectors) {
    ofstream efile ( "./logs/w0/sectorAngle.txt" );
    double maxRadius = this->radius;
    ...
    double startRadius, endRadius, radiusInc; //sector radii
    radiusInc = maxRadius /(1.*numSectors);
    startRadius = (numSectors-2)*radiusInc;
    endRadius = (numSectors-1)*radiusInc;
    ...
    for (int k=0;k<numSectors;k++) { 
      ...
      list<morph::Hex>::iterator bpi = this->hg->hexen.begin(); 
      efile << "begin hex "<<bpi->r <<" angle "<<bpi->phi<<endl;
      //efile << "end hex "<<this->hg->hexen.end()->r <<" angle "<<this->hg->hexen.end()->phi<<endl;
      double hexAngle = 0;
      while (bpi != this->hg->hexen.end()) {
	if (bpi->y >=  0)
	  hexAngle = bpi->phi;
	else
	  hexAngle = bpi->phi + 2*PI;
	if (bpi->r >= startRadius && bpi->r < endRadius) { 
         if (hexAngle >= startAngle && hexAngle < endAngle) { 
     	      angleCount[k]++; 
	      angleCC[k] = angleCC[k] + this->c[0][bpi->vi];
	      efile << "k " <<k << "  angle " << hexAngle << endl;
	      } //end of if on angle 
	 } //end of if on radius 
     	++bpi; 
      } //end of loop over bpi
       efile << "end hex "<<--bpi->r <<" angle "<< hexAngle<<endl;
