  ...
  Erm2009 (int scale, int offset, double z) {
	...
	for (int ix=-s; ix<=s; ix++) {
	  for (int jy=-s; jy<=s; jy++) {
        ...
        C.resize(n,0);
        for(int i=0;i<n;i++){
            N[i].resize(4,i); // CONNECT ALL TO 'boundary' UNIT AT N+1
            for(int j=0;j<n;j++){
                int dix = H[2][j]-H[2][i];
                int djy = H[3][j]-H[3][i];
		   if(max(abs(dix),abs(djy))==1){
                    //anticlockwise from east
                      if(dix== 1&&djy== 0){N[i][0]=j;C[i]++;}
                      if(dix==0&&djy== 1){N[i][1]=j;C[i]++;}
                      if(dix==-1&&djy==0){N[i][2]=j;C[i]++;}
                      if(dix==0&&djy==-1){N[i][3]=j;C[i]++;}
    ...
    vector<double> getLaplacian(vector<double> Q, double dx) {
      double overdxSquare = 1./(4.*dx*dx);
        vector<double> L(n,0.);
        for(int i=0;i<n;i++){
            L[i]=(Q[N[i][0]]+Q[N[i][1]]+Q[N[i][2]]+Q[N[i][3]]-4.*Q[i])*overdxSquare;
        }
        return L;
    }
...
for (int i=0;i<n;i++) {
            ...
            // METHOD 3
	  // double dix0N = (NN[N[i][0]]+NN[i])/2.;
	  // double djy0N = (NN[N[i][1]]+NN[i])/2.;
	  // double dix1N = (NN[N[i][2]]+NN[i])/2.;
	  // double djy1N = (NN[N[i][3]]+NN[i])/2.;
	  
	  // //double ncentre = NN[i];

          // double dix0C = CC[N[i][0]]-CC[i];
          // double djy0C = CC[N[i][1]]-CC[i];
          // double dix1C = CC[N[i][2]]-CC[i];
	  // double djy1C = CC[N[i][3]]-CC[i];

	  double dixN = NN[N[i][0]] - NN[N[i][2]];
	  double djyN = NN[N[i][1]] - NN[N[i][3]];
	  double dixC = CC[N[i][0]] - CC[N[i][2]];
	  double djyC = CC[N[i][1]] - CC[N[i][3]];
         

	  //finite volume for NdelC, h = s/2
	  //G[i] = (dix0N*dix0C+djy0N*djy0C+dix1N*dix1C+djy1N*djy1C)/(4.*ds*ds);
	  
	  

	  G[i] = (dixN*dixC+djyN*djyC)/(4.*ds*ds) + NN[i]*lapC[i];
...
int main (int argc, char **argv)
{
    ...
    double dt = .000001;            // integration timestep // SHOULD BE 0.001
    ...
    while (doing) {
        ...
        case 2: // *** PLOT ***
        {
		 ...
		 displays[0].drawTriFill(M.X[i],M.X[M.N[i][2]],M.X[M.N[i][3]],cl);
            }
            displays[0].redrawDisplay();
            break;
        }
