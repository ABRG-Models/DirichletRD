...
//	    input.close();
*/  
    cout << "expecting to read from h5 files" << endl;
    }
    else {
	    for (unsigned int i=0;i<S.Hgrid->num();i++) {
		    double choice = morph::Tools::randDouble();
		    if (choice > 0.5)
			  S.NN[i]=-(morph::Tools::randDouble())*1.0 + 1.;
		    else
			  S.NN[i]=(morph::Tools::randDouble())*1.0 + 1.;
		      S.CC[i]=(morph::Tools::randDouble())*1.0 + 2.5;
    ...
    cout <<  "just after field creation" << endl;
        ...
        array<float,3> c;
        c[2] = 0;
        c[0] = S.Hgrid->boundaryCentroid.first;
        c[1] = S.Hgrid->boundaryCentroid.second;
        cout << "d/2: " << S.Hgrid->hexen.begin()->d/4.0f << endl;
        disp.drawHex (c, offset, (S.Hgrid->hexen.begin()->d/2.0f), cl_a);
		 ...
		 if (i % numprint == 0) {
		  ...
		  int countHex = 0;
		   ...
		   for (unsigned int k=0;k<regsize;k++) {
             ...
             afile << S.NN[k] << "    " << normalNN[k] << " " << k <<endl;
			 }
