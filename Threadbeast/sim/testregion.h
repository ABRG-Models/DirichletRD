...
//go over the region and put the hexes into bins then average
              ...
              cout << "after edge loop Ecount " << Ecount << " Vcount " << Vcount <<endl;
			   ...
			   if (ihE.size() > 1) {
				 ...
				 //  if (hexRegionList[ihE[ii]][ihex] != central) {
                     hfile<<" central " <<central<< " nbr region " << hexRegionList[ihE[ii]][ihex]<< " hex " << ihE[ii] <<endl;
    ...
    morph::BezCurvePath regionFlow (int regNum) {
	  ...
	  vbIndex = this->regionVertex[regNum][(i+1)%polyDegree];
